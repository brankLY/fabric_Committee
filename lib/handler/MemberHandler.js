	...
	static async Init(stub, params) {
		...
		try {
			...
			for(const num in req){
				...
				logger.debug(' - Member %s not exists', createOption);
				if (exists) {
					logger.error(' - Member %s already exists', createOption);
					throw new Error(util.format('Member %s already exists', createOption.id));
				}

   ...
   */
	...
	static async checkCreateTxRequest(stub, options){
		...
		try {
			...
			logger.debug('Successfully created contractAccount %s', contractAccount.payload.toString('utf8'));
			let amount = contractAccount.payload.wallet.GZH.amount;
			logger.debug('Successfully created contractAccount amount %s', amount);

   ...
   */
	...
	static async checkCreateTxRequest(stub, options){
		...
		try {
			...
			let account = contractAccount.payload.toString('utf8');
			account = JSON.parse(amount);

			const wallet = account.wallet;
			logger.debug('Successfully query contractAccount wallet %j', wallet);
			logger.debug('Successfully created contractAccount amount %s', account.wallet.GZH.amount);

			const amount = Number(account.wallet.GZH.amount);

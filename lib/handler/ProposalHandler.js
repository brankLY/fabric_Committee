...
class ProposalHandler {
   ...
   */
	static async Create(stub, params) {
		...
		try {
			logger.enter(method);
			if (!params) {
   				logger.error('%s - Create new User Proposal params of length 1, found %s, %j', method, params);
  				throw new Error('Create new Proposal requires params of length 1');
 		    }
			logger.debug('%s - params: %j', method, params);
			if (params.length !== 1) {
				logger.error('%s - Create new User Proposal params of length 1, found %s, %j', method, params.length, params);
			...
			if (proposals){
				...
				logger.debug(' - none proposals are: %j', proposals);
				proposals = [];
			}
	...
	static async ChangeMemberProposal(stub, params) {
		const method = 'ChangeMemberProposal';
		try {
			logger.enter(method);
			if (!params) {
   				logger.error('%s - Create new User Proposal params of length 1, found %s, %j', method, params);
			...
			if (memberProposals){
				...
				logger.debug(' - none memberProposals are: %j', memberProposals);
				memberProposals = [];
			}
			const identityService = new IdentityService(stub);
			const id = identityService.getName();
			...
			const proposal = new Proposal(stub);
			logger.debug('%s - Member proposal is %j', method, createProposalRequest);
			await proposal.createMemberProposal(createProposalRequest);
   ...
   */
	...
	static async GetAll(stub) {
		...
		let proposals;
		try {
			proposals = (await stub.getState('d')).toString('utf8');

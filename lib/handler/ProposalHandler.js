   ...
   */
	static async Vote(stub, params) {
		...
		try {
			...
			const proposal = new Proposal(stub);
			let targetProposal = await proposal.getOne(createVoteRequest.proposalId);
			// targetProposal = JSON.parse(targetProposal);
			logger.debug('%s - targetProposal: %j', method, targetProposal);
			await proposal.doVote(createVoteRequest, targetProposal);

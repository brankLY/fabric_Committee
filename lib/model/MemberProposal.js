   ...
   */
	...
	async doVote(tx) {
		...
		try {
			...
			const exists = await member.exists(tx.accountId);
			...
			if (targetProposal.currentResult >= math.divide(10,3)){
				if (targetProposal.type === 'create') {
					await this.executeProposalCreate(tx);
				}
				else if (targetProposal.type === 'remove') {
					await this.executeProposalRemove(tx);
				}
			}
			await this.stub.putState(this.key, Buffer.from(JSON.stringify(targetProposal)));
			/**
	...
	async executeProposalCreate(option) {
		...
		try{
			...
			const targetProposal = await this.getOne(option.proposalId);
   ...
   */
	...
	async checkTime(options) {
		...
		try {
			...
			logger.debug('%s - endTime is  %s %s', method, typeof(endTime));
			if (endTime === 'Invalid Date'){
				logger.error('targetProposal %s `s deadline is illegal', options.name);
				throw new Error(util.format('argetProposal %s `s deadline is illegal', options.name));
			}
			const now = new Date(Date.now());
			logger.debug('%s - endTime is  %s, now is %s', method, endTime, now);
			if (endTime <= now) {
				logger.error('targetProposal %s already end', options.name);
				throw new Error(util.format('targetProposal %s already end', options.name));
			}
			logger.exit(method);
			return true;
		} catch (e) {

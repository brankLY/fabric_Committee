...
class Member extends BaseModel{
	...
	validateOptions(fcn, options) {
		...
		case 'create':
			...
			logger.debug('%s - Member %s not exists', method, options);
			if (exists) {
			logger.error('%s - Member %s already exists', method, memberID);
			throw new Error(util.format('Member %s already exists', memberID));
			}
			break;
		case 'update':

...
class Member extends BaseModel{
   ...
   */
	static async GetValidity(stub, userId) {
	...
	async exists(id) {
		...
		logger.debug('%s - member with id:%s exists:%s', method, id, res);
		logger.exit(method);
		return res;
	}

	static async Create(stub, id) {
		const method = 'Create';
		logger.enter(method);

		const memberID = id;

		Member.CHECK_CREATE_OPTIONS(memberID);
		logger.debug('%s - Pass CREATE Validation', method);

		this.checkAdminPermission(stub, memberID);
		const exists = await this.exists(memberID);
		logger.debug('%s - User %s exists: %s', method, memberID, exists);
	...
	static async Save(stub, memberObj) {
		this.CHECK_SAVE_OPTIONS(memberObj);
		const member = this.FROM_JSON(stub, memberObj);
		logger.debug(' - Result member Check in Save %j', member);
		return member.save();
	}

	static BUILD_MEMBER_KEY(id) {
		return this.stub.createCompositeKey(this.prefix, [id]);
	}

	async checkAdminPermission(stub, options) {
		let account = await stub.invokeChaincode(EARTH_CHAINCODE_ID, ['account.query']);
		account = account.payload.toString('utf8');
		logger.debug('- toString(account) in member %s', account);
		account = JSON.parse(account);
		logger.debug('%j - JSON.parse in member %s', account, account);

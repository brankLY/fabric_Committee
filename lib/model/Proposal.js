...
class Proposal extends BaseModel {
   ...
   */
	async doCreate(options) {
		...
		this.deadline = this.setDeadLine(options.deadline);
		this.timestamp = this.stub.getTxTimestamp();
	...
	async doVote(tx) {
		...
		try {
			...
			this.checkVoteOptions(tx);
			logger.debug('update proposal by tx:%j', tx);
			this.buildKey(tx.proposalId);
			...
			if (timestamp > targetProposal.deadline) {
   ...
   */
	async checkCreateOptions(options) {
		const fields = [
			...
			{ name: 'deadline', type: 'uint4', required: true },
		];

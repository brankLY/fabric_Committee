...
class Proposal extends BaseModel {
   ...
   */
	async checkCreateOptions(options) {
		...
		SchemaCheker.check(fields, options);
		// const target = await this.stub.invokeChaincode(EARTH_CHAINCODE_ID, ['account.query', options.target]);
		// if (!target) {
		// 	logger.error(' - account %s not exists', options.target);
		// 	throw new Error('account '+options.target+' not exists');
		// }
	}
